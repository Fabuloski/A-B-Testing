...
using CSV, DataFrames, CairoMakie, Permutations, StatsBase
...
# ╔═╡ 0c6e07ab-8c79-4e3c-ac94-e9174d18cdf7
update_theme!(
	resolution=(500, 500), 
	font="Noto Sans", 
	linewidth=3, 
...
mkdir("back_pain_project")

# ╔═╡ 3a1ee822-c0a2-4510-b703-d0b5b194c3db
begin
	url = "https://raw.githubusercontent.com/data-8/textbook/main/assets/data/bta.csv"
	download(url, joinpath("back_pain_project", "back_pain.csv"))
	back_pain_data = CSV.read("back_pain_project\\back_pain.csv", DataFrame)
end
...
# ╔═╡ d9b6beff-9d70-45d0-8841-a88430466b8f
function frac(Result)
	len = length(Result)
	relief = sum(Result .== 1)
	frac = relief/len
	return frac
...
begin
	data = combine(groupby(back_pain_data, :Group), :Result => frac)
	rename!(data, :Result_frac=> "Fraction with pain relief")
end

# ╔═╡ 441cf340-bfab-447f-9c1a-d8022def5f47
...
begin
	...
	ax =Axis(fig[1, 1], xlabel = "Treatment", ylabel = "Number", 
			 xticks = (1:nrow(data), data[:, "Group"]), 
			 title = "Fraction of people with pain relief")
	
	barplot!(1:nrow(data), data[:, "Fraction with pain relief"])
...
md"
_null hypothesis_: The treatment does not have an effect

_alternative hypothesis_: The treatment has an effect
"
...
md"
_test statistic_:= The absolute difference in the ratio of people with relief for both groups
...
begin
	order = sample(1:31, 31, replace=false)
	back_pain_data[:, "Sim Group under Null"] = back_pain_data[:, "Group"][order]
end
...
function proportion_w_pain_relief(data::DataFrame, group_col::String, group::String)
	filter_data = filter(group_col => col -> col .== group, data)
	data = frac(filter_data[:, "Result"])
	return data
end
...
proportion_w_pain_relief(back_pain_data, "Group", "Treatment")

# ╔═╡ 62794052-25c8-4834-aeec-8ab8ce2bb680
data[:, "Fraction with pain relief"]
...
function test_statistic(data::DataFrame, group_col::String)
	control = proportion_w_pain_relief(data, group_col, "Control")
	treatment = proportion_w_pain_relief(data, group_col, "Treatment")
...
truth = test_statistic(back_pain_data, "Group")
...
function simulate(data::DataFrame, n_sims::Int64 =10000)
	sims= zeros(n_sims)
	for i in 1:n_sims
		 order = sample(1:31, 31, replace=false)
		 data[:, "Sim Group under Null"] = data[:, "Group"][order]
		sims[i] = test_statistic(data, "Sim Group under Null")
	end
	return sims
end

# ╔═╡ 62c9aa82-bd39-4124-9816-afb44e1e5f81
sim = simulate(back_pain_data)

# ╔═╡ 4c5d30c2-0e59-11eb-3987-3bc356bb1f66
begin
	fig2 = Figure()
	ax2 = Axis(fig2[1,1], xlabel = "difference", ylabel = "frequency")
	f = round(Int, maximum(sim)/0.1)
	hist!(sim, bins=f)
	vlines!(truth, color=:red, linestyle = :dashdot)
	fig2
end

# ╔═╡ fd0aef3a-8011-42c6-acb2-9d7facd735b7
truth

# ╔═╡ 5b2e1530-0e59-11eb-055a-e38373f7005d
begin
	prob = [i >= truth for i in sim]
	p = sum(prob)/ length(sim)
end
...
* I am assuming a significance value of 0.01 because this is a medical field and we want to be more certain about the probability of observing the same result provided the null hypothesis is true. 
...
* I chose a one-sided hypothesis because it is easy to say if this threshold is not meant, we are in the alternative domain. A two-sided hypothesis is just the union of two one-sided hypothesis tests. 
...
* There is no assurance that the drug causes pain relief, all we got from the test is that the null hypothesis can be rejected in favor of the alternative and that the drug does cause an effect.
...
* No, it won't be evidence that the drug causes pain relief because there are many variables to consider. What if the treatment group patient has a gene that makes them heal sooner or if there is a huge age gap between the two groups where one age group has a higher tendency of regenerating faster than the other group? 
"
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "06146c09b3f660fedc0ec7e2259acc6a9897c439"
...
git-tree-sha1 = "4ce9ab93e7dbaaca4777dbe5b3f805c32fab5ee2"
